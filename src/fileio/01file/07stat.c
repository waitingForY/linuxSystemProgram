...
char* getuname(struct stat* buf)
{
	struct passwd* passwdbuf=getpwuid(buf->st_uid);
	return passwdbuf->pw_name;
}

char* getgname(struct stat* buf)
{
	struct group* groupbuf=getgrgid(buf->st_gid);
	return groupbuf->gr_name;
}

char* getupwd(struct stat* buf)
{
	struct passwd* passwdbuf=getpwuid(buf->st_uid);
	return passwdbuf->pw_passwd;
}
int main(int argc,char **argv)
{
	...
	printf("用户名：%s,(%d),密码：%s\n",getuname(&statbuf),(int)statbuf.st_uid,getupwd(&statbuf));
